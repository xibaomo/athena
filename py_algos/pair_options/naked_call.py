...
def calibrate_strike(cur_price, calls, steps, lb_rtn , ub_rtn, cdf_cal):
    max_rtn = -9990.0
    best_strike = 0.0
    ...
    for call in calls:
        ...
        if strike_rtn >= ub_rtn or strike_rtn <= lb_rtn:
            continue
        bid = float(call['bid'])
        exp_rtn = compExpectedReturn(cur_price,strike,bid,probs,drtn,lb_rtn)
        # breakpoint()
        idx = int(((strike/cur_price-1.)-lb_rtn)/drtn)
        assign_prob = np.sum(probs[idx:])
        print(f"strike: {strike}, asgn prob: {assign_prob:.3f}, exp_rtn: {exp_rtn:.4f}, bid: {bid},"
              f"bid*prob: {bid*(1-assign_prob):.3f}")
        if assign_prob < 0.05:
            break
        if exp_rtn > max_rtn:
            max_rtn = exp_rtn
...
if __name__ == '__main__':
    ...
    best_strike,max_rtn = calibrate_strike(cur_price,calls,steps,lb_rtn = -0.6, ub_rtn = 1.,cdf_cal=cdf_cal)
    ...
    best_strike, max_rtn = calibrate_strike(cur_price, calls, steps, lb_rtn=-0.6, ub_rtn=1.,
                                                cdf_cal=cdf_cal)
    print(f"Latest price: {cur_price:.2f}")
    print(f"best strike: {best_strike}, max_rtn: {max_rtn}, exp_profit: {best_strike * max_rtn:.2f}")
    print(f"max daily return: {max_rtn / fwd_days:.4f}, annual return: {max_rtn / fwd_days * 252:.4f}")
